...
import arquivoAutomovel from "@/pages/automoveis/edicao/apoio/arquivoAutomovel.vue";
import InformacoesBasicas from "@/pages/automoveis/edicao/apoio/informacaoBasica.vue";
import ItemAutomovel from "@/pages/automoveis/edicao/apoio/itemAutomovel.vue";
import siteAutomovel from "@/pages/automoveis/edicao/apoio/siteAutomovel.vue";
...
onMounted(() => {
  auto.AutomovelMostrar(route.params.id);
  auto.TipoListar();
  auto.CategoriaListar();
  auto.MarcasListar();
  auto.ModeloListar(auto.edicao.marca_automovel[0].id);
});
...
const addItemToArray = (item) => {
  const Items = {
    id: auto.edicao.itens.length + 1,
    nome_item: item,
    automovel_id: 1, //
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  };
  auto.edicao.itens.push(Items);
};
...
const submit = () => {
  auto.AutomovelEdicao(route.params.id);
};
</script>
