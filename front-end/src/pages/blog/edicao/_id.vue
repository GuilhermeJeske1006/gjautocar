...
const accountDataLocal = ref(
  structuredClone(JSON.parse(JSON.stringify(blog.edicao)))
);
const isAccountDeactivated = ref(false);

const resetForm = () => {
  accountDataLocal.value = structuredClone(
    JSON.parse(JSON.stringify(blog.edicao))
  );
};
...
const validate = () => {
  if (blog.edicao.titulo == "" || blog.edicao.subtitulo == "") {
    if (blog.edicao.titulo == "") {
      error.erroTitulo = "Preencha o campo titulo do blog";
    }
    if (blog.edicao.subtitulo == "") {
...
onMounted(() => {
  blog.BlogMostrar(route.params.id);
});

const submit = () => {
  if (validate()) {
    blog.BlogEditar(route.params.id);
  }
};
  ...
  if (files && files.length) {
    ...
    fileReader.onload = () => {
      if (typeof fileReader.result === "string") {
        ...
        blog.edicao.foto = fileReader.result;
      }
    };
  }
};
